...
interface BrowserAgentProps {
  ...
  /**
   * When false, the browser keeps running (tabs/iframe/automation) but the
   * full-screen UI is hidden — used for voice-driven background browsing.
   */
  visible?: boolean;
  actionTrigger?: {
    type: string;
...
export const BrowserAgent: React.FC<BrowserAgentProps> = ({
  ...
  /**
   * Sites that cannot load in a nested iframe even via proxy.
   * YouTube is NOT restricted — watch URLs become embeds; home/search use in-built UI.
   * Soft-restricted sites still attempt proxy load so background browsing can work.
   */
  const checkIsRestricted = (urlStr: string): { restricted: boolean; reason: string } => {
    ...
    try {
      ...
      // YouTube / youtu.be: handled in-built (embed + results UI) — never block
      if (hostname.includes("youtube.com") || hostname.includes("youtu.be")) {
        return { restricted: false, reason: "" };
      }

      // Prefer DuckDuckGo HTML proxy for Google home (credential pages stay soft-restricted)
      if (hostname.includes("google.com") && !parsed.pathname.includes("/search") && parsed.pathname !== "/") {
        return {
          restricted: true,
          reason: "Google account/console pages block nested frames.",
        };
      }
      if (hostname.includes("chatgpt.com") || hostname.includes("openai.com")) {
        return {
          restricted: true,
          reason: "OpenAI blocks nested frames (auth / Cloudflare).",
        };
      }
      if (hostname.includes("gmail.com") || hostname.includes("mail.google.com")) {
        return {
          restricted: true,
          reason: "Gmail blocks nested frames for security.",
        };
      }
      // Everything else: try in-built proxy/iframe (background-safe)
      return { restricted: false, reason: "" };
    } catch {
      return { restricted: false, reason: "" };
    }
  ...
  /** Normalize voice/tool URLs so they stay inside the in-built browser. */
  const normalizeInbuiltUrl = (raw: string): string => {
    let finalUrl = (raw || "").trim();
    if (!finalUrl || finalUrl === "about:blank") return finalUrl || "about:blank";
    ...
    try {
      ...
      // Bare YouTube home cannot load in an iframe. Keep a real search URL with a
      // non-empty query so /api/youtube-search never hits empty-q / connection errors.
      // Prefer openWebsite (system browser) for simple "open YouTube" voice commands.
      if (
        (host.includes("youtube.com") || host === "youtu.be") &&
        (u.pathname === "/" || u.pathname === "" || u.pathname === "/feed" || u.pathname === "/feed/trending")
      ) {
        return "https://www.youtube.com/results?search_query=trending";
      }
      if (host === "youtu.be" && u.pathname.length > 1) {
        return `https://www.youtube.com/watch?v=${u.pathname.replace(/^\//, "").split("/")[0]}`;
      }
      // Bare Google → DuckDuckGo HTML (iframe-friendly)
      if (host.includes("google.") && (u.pathname === "/" || u.pathname === "")) {
  ...
  // Safe tab initialization (only seeds the first tab once; later navigations use actionTrigger)
  useEffect(() => {
    if (!initialUrl) return;
    // Don't reset tabs if we already have an active session (background remounts)
    if (tabs.length > 0) return;

    const startUrl =
      initialUrl === "about:blank" ? "about:blank" : normalizeInbuiltUrl(initialUrl);
  ...
  // Tracks the newest search request so a slow older response cannot overwrite
  // results for the CURRENT query (stale-fetch race when the user searches again).
  const ytSearchRequestRef = useRef(0);
  ...
  useEffect(() => {
    if (activeTab) {
      ...
      if (activeTab.url.includes("youtube.com/results")) {
        const requestId = ++ytSearchRequestRef.current;
        ...
        try {
          ...
          fetch(`/api/youtube-search?q=${encodeURIComponent(q)}`)
            ...
            .then(data => {
              if (ytSearchRequestRef.current !== requestId) return; // stale — drop
              setYtSearchResults(data.results || []);
              setYtSearchLoading(false);
              // Complete loading state cleanly — use captured tabId to avoid stale closure
              setTabs(prev => prev.map(t => t.id === currentTabId ? { ...t, isLoading: false } : t));
            })
            .catch(err => {
              if (ytSearchRequestRef.current !== requestId) return; // stale — drop
          ...
          if (ytSearchRequestRef.current !== requestId) return;
          setYtSearchError("Invalid YouTube search URL structure.");
          setYtSearchLoading(false);
        }
      } else {
        ytSearchRequestRef.current++; // invalidate any in-flight fetch for the old page
        setYtSearchResults([]);
      }
    }
  ...
  useEffect(() => {
    const handleNavigationMessage = (event: MessageEvent) => {
      // Only the /api/web-proxy interceptor (served from this origin) may drive
      // navigation. Without this check any third-party frame — e.g. the YouTube
      // embed — could post a NAVIGATE and redirect the in-built browser.
      if (event.origin !== window.location.origin) return;
      if (event.data && event.data.type === "NAVIGATE" && event.data.url) {
        console.log("[Bikli Browser] Same-origin child iframe navigated to:", event.data.url);
  ...
  useEffect(() => {
    ...
    console.log(`[Bikli Browser Hub] Automated Voice Trigger: ${type}`, args);

    const runVoiceAutomation = async () => {
      try {
        switch (type) {
          case "browserOpen": {
            const destUrl = normalizeInbuiltUrl(args.url || "https://html.duckduckgo.com/html/");
            navigateToUrl(destUrl);
            const cleanTitle = getCleanTitleFromUrl(destUrl);
            callback({
              result: `Opening ${cleanTitle} in the background browser for you now.`,
            });
            break;
          }
          case "browserSearch": {
          ...
          case "browserTabAction": {
            ...
            if (tabAction === "new") {
              ...
              // Unknown sub-action — must still answer or Gemini hangs waiting.
              callback({
                error: `Unknown browserTabAction "${String(tabAction || "")}". Supported: new, close, switch.`,
              });
            }
            break;
          }
          case "browserScroll": {
            ...
            // Mouse-wheel style: ~100px per notch, default 3 notches (not a full page)
            let notches = Number(args.amount ?? 3);
            if (!Number.isFinite(notches)) notches = 3;
            if (notches >= 50) notches = Math.max(2, Math.min(8, Math.floor(notches / 120)));
            notches = Math.max(1, Math.min(8, Math.abs(Math.round(notches))));
            const amount = notches * 100;
            const iframe = iframeRef.current;
            if (iframe && iframe.contentWindow) {
              ...
              callback({
                result: `Scrolled ${direction} a little (mouse wheel ×${notches}).`,
              });
            } else {
          ...
          case "browserClick": {
            const selector = String(args.selector || args.description || args.target || "").trim();
            const selLower = selector.toLowerCase();

            // "play first video" / "first result" — use in-built YouTube results list
            // ONLY while the active tab is still a results page (never stale old search).
            const onYtResults =
              Boolean(activeTab?.url?.includes("youtube.com/results")) &&
              ytSearchResults.length > 0;
            const wantsFirstVideo =
              /first\s*(video|result|one)?/.test(selLower) ||
              selLower === "first" ||
              selLower === "play" ||
              selLower === "play-first" ||
              selLower === "play first" ||
              selLower.startsWith("video-0") ||
              selLower === "video-1";

            if (wantsFirstVideo && onYtResults) {
              const hit = ytSearchResults[0];
              const watchUrl = `https://www.youtube.com/watch?v=${hit.videoId}`;
              navigateToUrl(watchUrl);
              callback({
                result: `Playing the first video: "${hit.title || hit.videoId}".`,
                url: watchUrl,
                videoId: hit.videoId,
              });
              break;
            }
            // video-N or index N from YouTube results UI
            const videoIdxMatch = selLower.match(/^video-(\d+)$/) || selLower.match(/^(?:#|number\s*)?(\d+)(?:st|nd|rd|th)?\s*video/);
            if (videoIdxMatch && onYtResults) {
              const n = Math.max(0, parseInt(videoIdxMatch[1], 10) - (selLower.startsWith("video-") ? 0 : 1));
              const idx = selLower.startsWith("video-") ? parseInt(videoIdxMatch[1], 10) : n;
              const hit = ytSearchResults[Math.max(0, idx)] || ytSearchResults[0];
              if (hit) {
                const watchUrl = `https://www.youtube.com/watch?v=${hit.videoId}`;
                navigateToUrl(watchUrl);
                callback({
                  result: `Playing: "${hit.title || hit.videoId}".`,
                  url: watchUrl,
                  videoId: hit.videoId,
                });
                break;
            ...
            if (iframe && iframe.contentWindow) {
              const doc = iframe.contentWindow.document;
              let element = (selector ? doc.querySelector(selector) : null) as HTMLElement | null;
              if (!element && selector) {
            ...
            } else if (wantsFirstVideo && onYtResults) {
              const hit = ytSearchResults[0];
              navigateToUrl(`https://www.youtube.com/watch?v=${hit.videoId}`);
              callback({ result: `Playing the first video: "${hit.title || hit.videoId}".` });
            } else {
  ...
  // Navigates active tab to location (always stays in-built; never system browser)
  const navigateToUrl = (targetUrl: string) => {
    let finalUrl = normalizeInbuiltUrl(targetUrl.trim());
    ...
    // Ensure a tab exists (background engine may receive open before seed finishes)
    ...
    if (activeTab) {
      ...
      try {
        ...
        if (iframe && iframe.contentDocument) {
          ...
          if (bodyTxt.includes("Bikli Web Proxy Error") || bodyTxt.includes("Failed loading remote website")) {
  ...
  // Background mode: engine stays alive (iframe + voice automation) but UI is off-screen.
  const shellClass = visible
    ? "fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl animate-fade-in text-left select-none"
    : "fixed left-[-12000px] top-0 w-[1280px] h-[800px] opacity-0 pointer-events-none overflow-hidden z-[-1] text-left";

  const panelClass = visible
    ? "relative w-full max-w-5xl h-[88vh] flex flex-col rounded-3xl border border-white/10 bg-slate-900/85 shadow-[0_0_90px_rgba(168,85,247,0.4)] overflow-hidden"
    : "relative w-full h-full flex flex-col bg-slate-900 overflow-hidden";

  return (
    <div
      id="bikli-playwright-automation-hud"
      className={shellClass}
      aria-hidden={!visible}
      data-background={!visible ? "true" : "false"}
    >
              ...
              <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center justify-center space-y-8 select-none text-center scrollbar-none">
                ...
                <div className="p-3 border border-white/5 bg-slate-950/50 rounded-2xl max-w-md w-full">
                  <p className="text-[9px] font-mono text-slate-500 leading-normal uppercase">
                    🌸 COMPANION TIP: Voice command Bikli to trigger any automation: <br />
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-sans text-slate-400 leading-relaxed max-w-md mx-auto">
                      Bikli automatically detected secure sandbox constraints for <span className="font-mono text-indigo-300 font-semibold">{getCleanTitleFromUrl(activeTab?.url || "")}</span>. 
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-sans text-slate-400 leading-relaxed max-w-sm mx-auto font-medium">
                      Bikli was unable to load <span className="font-mono text-indigo-300 font-semibold">{getCleanTitleFromUrl(activeTab?.url || "")}</span> securely. This website may be offline, spelled incorrectly, or blocking automated server access.
                    </p>
                  </div>
                ...
                <div className="px-6 py-4.5 border-b border-white/5 bg-slate-900/40 flex items-center justify-between z-10">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs text-slate-200 font-bold uppercase tracking-wider">
                      Real-time YouTube Portal: &ldquo;{(() => { try { return new URL(activeTab.url).searchParams.get("search_query") || ""; } catch { return ""; } })()}&rdquo;
                    </span>
                  </div>
                  ...
                  {!isLocalConnected ? (
                    ...
                    <div className="flex-1 overflow-y-auto space-y-5 pr-1 select-text">
                      <div className="p-3.5 rounded-xl border border-indigo-500/10 bg-indigo-500/10 text-[11px] font-sans leading-normal text-indigo-300">
                        ...
                        <span>If you prefer to command a real, headed Chrome browser on your desktop computer, easily launch Bikli's local client script!</span>
                      </div>
                    ...
                    <div className="flex-1 overflow-y-auto font-mono text-[10px] space-y-2 pr-1 select-text scrollbar-thin">
                      {localLogs.length === 0 ? (
                        <div className="text-slate-600 italic">No operations recorded yet. Speaking with Bikli will generate real terminal logs.</div>
                      ) : (
                        localLogs.map((log) => (
                          <div
                            key={log.id}
              ...
              <div className="p-4 grid grid-cols-1 md:grid-cols-4 gap-4 font-mono text-[10px] tracking-wide text-slate-400 leading-normal max-h-48 overflow-y-auto select-text scrollbar-thin">
                ...
                <div className="md:col-span-4 flex items-center justify-between border-b border-white/5 pb-1.5 mb-1 text-[11px] font-extrabold uppercase text-amber-400">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>Active Bikli Browser Diagnostics console</span>
                  </div>
                ...
                <div className="space-y-1 bg-white/5 p-2 rounded-xl border border-white/5 md:col-span-1">
                  ...
                  <div className="max-h-16 overflow-y-auto scrollbar-thin text-[9px] text-slate-500 select-text leading-normal">
                    {jsErrors.length === 0 ? (
