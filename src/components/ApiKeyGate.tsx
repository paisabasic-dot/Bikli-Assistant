 ...
 *
 * BIKLI ships without any API key. On launch we ask the backend whether a key
...
export function ApiKeyGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function checkConfig(attemptsLeft: number): Promise<void> {
      try {
        ...
        if (cancelled) return;
        if (attemptsLeft > 1) {
          // Backend may still be starting — wait 800 ms and retry.
          await new Promise((r) => setTimeout(r, 800));
          return checkConfig(attemptsLeft - 1);
        }
        // All retries exhausted — assume no key configured so user can enter one.
        setPhase("needsKey");
      }
    }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#050509] text-white">
      ...
      {phase === "checking" ? (
        <div className="flex flex-col items-center gap-4 text-white/60">
          ...
          <span className="text-sm tracking-wide">Starting BIKLI…</span>
        </div>
      ) : (
        <form
          onSubmit={submit}
        ...
        >
          <div className="mb-6 flex flex-col items-center text-center">
            ...
            <h1 className="text-xl font-semibold tracking-tight">Welcome to BIKLI</h1>
            <p className="mt-2 text-sm leading-relaxed text-white/55">
              BIKLI runs on your own Google Gemini API key. Paste it below to get
