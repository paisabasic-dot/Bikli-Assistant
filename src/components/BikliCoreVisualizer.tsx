...
export const BikliCoreVisualizer: React.FC<BikliCoreVisualizerProps> = ({
  ...
  // Track per-clip errors so one transient decode failure cannot permanently
  // cover the whole character with the "AWAITING VIDEOS CORES" overlay. The
  // fallback only appears when ALL three clips are genuinely missing.
  const [videoErrors, setVideoErrors] = useState<Record<string, boolean>>({});

  const handleVideoError = (videoName: string) => {
    console.warn(`[Bikli Web Video] Failed to load video source for: ${videoName}`);
    setVideoErrors((prev) => ({ ...prev, [videoName]: true }));
  };

  const hasError = Boolean(videoErrors.idle && videoErrors.thinking && videoErrors.talking);
  ...
  // Prop-tracking refs for a persistent, flicker-free canvas loop.
  // Reading the live props every frame would re-create requestAnimationFrame
  // on every state change (listening↔speaking churn) — that caused the
  // character to re-initialize / flicker constantly during conversation.
  ...
  // Keep all clips looping; only opacity switches states.
  // Never seek / restart / scale — that was the face "zoom pop" on tool actions.
  useEffect(() => {
    const ensurePlaying = (videoEl: HTMLVideoElement | null) => {
      if (!videoEl) return;
      try {
        if (videoEl.paused) {
          ...
          if (playPromise !== undefined) {
            playPromise.catch(() => {
              /* autoplay blocked — muted playsInline should still work */
            });
          }
        }
      } catch {
  ...
  const getGlowColors = (theme: string) => {
    switch (theme) {
      case "violet":
  ...
  // Main high speed Canvas graphics rendering loop (persistent — one rAF loop)
  useEffect(() => {
    ...
    // Reused per-frame audio analysis buffer (kept outside the rAF callback so
    // it isn't re-allocated 60x/sec — getByteFrequencyData overwrites it).
    ...
    const render = () => {
      ...
      const colors = getGlowColors(themeRef.current);

      // Dynamic audio frequency fetching from real voice session
      let audioLevel = 0;
      let activeAnalyser = null;

      if (stateRef.current === "speaking" && sessionRef.current?.outputAnalyser) {
        activeAnalyser = sessionRef.current.outputAnalyser;
      } else if (stateRef.current === "listening" && sessionRef.current?.inputAnalyser) {
        activeAnalyser = sessionRef.current.inputAnalyser;
      }
      ...
      // ==========================================
      const applyGlitch = (stateRef.current === "connecting" && Math.random() < 0.1) || (Math.random() < 0.005);
      if (applyGlitch) {
        ctx.save();
    ...
    // Single long-lived loop — state/session/theme are read from refs above,
    // so changes to listening/speaking/emotion never restart (flicker) the
    // renderer mid-conversation.
  }, []);
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      <div
        id="bikli-animated-presence"
        className="bikli-presence-float absolute z-10 w-full h-full flex items-center justify-center pointer-events-auto transition-all duration-700"
      >
        <div className="relative w-full max-w-4xl aspect-[4/3] flex items-center justify-center scale-[0.95] sm:scale-105 translate-y-6 md:translate-y-10 overflow-hidden select-none pointer-events-none md:max-h-[78vh] max-h-[68vh]">
          ...
          {/* Animated neon border (blue → cyan → purple flowing) */}
          <div className="bikli-card-border-anim" />
      ...
      <canvas
        id="bikli-hologram-living-canvas"
        ref={canvasRef}
