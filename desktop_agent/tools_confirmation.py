...
Step 1: requestPowerAction(action) -> mints a single-use, short-lived token
        and tells BIKLI (via the result string) to ask the user to confirm.
...
# Friendly human labels so BIKLI's prompt-to-confirm reads naturally.
ACTION_LABEL = {
...
def _purge_expired() -> None:
    now = time.time()
    # Guard the dict — /execute handlers run concurrently in FastAPI's threadpool.
    with STATE.lock:
...
def request_power_action(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    BIKLI calls this first; the returned message instructs her to ask the
...
def consume_token(action: str, token: str | None) -> None:
    ...
    # Atomic get-and-pop under lock to prevent race conditions.
    with STATE.lock:
