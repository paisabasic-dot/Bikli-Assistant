/**
 * Bikli Playwright Local Agent Server
 * Run this locally on your machine to grant Bikli real control over your browser!
 *
 * Setup instructions:
 ...
 * 4. Edit package.json to add: "type": "module"
 * 5. Run: npm install playwright express cors
 * 6. Run: npx playwright install chromium
 * 7. Launch the server: node local-agent.js
 *
 * NOTE: This file uses ES Module syntax (import/export). Your package.json
 * must contain "type": "module" for Node.js to load it correctly. If you
 * prefer CommonJS, replace all "import" with "require" calls.
 *
 * This server binds to port 3001 on localhost, permitting Bikli's web portal to issue
...
let logsList = [];
// In-flight guard so concurrent /api/action requests share one browser launch.
...
// browserPromise memoizes the launch so concurrent /api/action requests cannot
// each spin up their own Chromium instance (leaking headed browsers).
async function ensureBrowser() {
  if (!browserPromise) {
    browserPromise = (async () => {
      ...
      // Allow a later request to retry the launch.
      browserPromise = null;
      throw err;
    });
  }
...
app.post("/api/action", async (req, res) => {
  ...
  try {
    ...
    switch (type) {
      ...
      default:
        throw new Error(`Directive '${type}' not recognized by Bikli's local Playwright engine.`);
    }
...
// Bind to loopback only — this API drives the user's real browser (open, click,
// type, read the current URL) with no authentication, so it must never be
// reachable from the LAN. An 'error' handler keeps a port conflict from
// crashing the process with an unhandled exception.
const server = app.listen(PORT, "127.0.0.1", () => {
  console.log(`\n======================================================`);
  console.log(`🚀 Bikli Playwright Local Agent Server Running!`);
  console.log(`📡 Listening on: http://127.0.0.1:${PORT}`);
  console.log(`======================================================\n`);
});
server.on("error", (err) => {
  if (err && err.code === "EADDRINUSE") {
    console.error(`[error] Port ${PORT} is already in use. Is another Bikli Local Agent already running?`);
  } else {
    console.error(`[error] Server failed to start: ${err ? err.message : err}`);
  }
  process.exit(1);
});
